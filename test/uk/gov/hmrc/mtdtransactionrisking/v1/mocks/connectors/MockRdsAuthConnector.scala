...
trait MockRdsAuthConnector extends MockFactory:
  ...
  val mockRdsAuthConnector: RdsAuthConnector = mock[RdsAuthConnector]

  object MockRdsAuthConnector:

    def retrieveBearerToken: CallHandler[Future[ServiceOutcome[RdsAuthCredentials]]] =
      (mockRdsAuthConnector
        .retrieveBearerToken()(_: HeaderCarrier, _: CorrelationId))
        .expects(*, *)
